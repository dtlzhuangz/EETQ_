...
def ppl_eval():
    ...
    ts_model = AutoModelForCausalLM.from_pretrained(model_dir, config=config, trust_remote_code=True)
    ...
    for i in range(len(testdata)):
        ...
        input_data.append(tokenizer(s[:idx + 16], return_tensors='pt').input_ids[0].unsqueeze(0))
        output_data.append(tokenizer(s[idx + 16:], return_tensors='pt').input_ids[0].unsqueeze(0))
